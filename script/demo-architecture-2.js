...
(function ($) {
  ...
  $slider.on("changed.owl.carousel", function (e) {
    $(".custom-slider-background .custom-slider-background-image-stage").each(
      function () {
        var $stage = $(this),
          $stageOuter = $stage.closest(
            ".custom-slider-background-image-stage-outer"
          ),
          $currentItem = $stage
            .find(".custom-slider-background-image-item")
            .eq(e.item.index),
          nItems = $stage.find(".custom-slider-background-image-item").length;

        var distance = $stage.hasClass("reverse")
            ? $currentItem.outerHeight() * nItems -
              $currentItem.outerHeight() * (e.item.index + 1)
            : $currentItem.outerHeight() * e.item.index,
  ...
  var loadMore = {
    ...
    loadMore: function () {
      ...
      $.ajax({
        url:
          "ajax/demo-architecture-2-ajax-projects-load-more-" +
